---
title: "CAP Biodiversity Dynamics - Bird Data Prep"
...
write.csv(bird.list.joined, "C:/Research/CAPLTER/data/core_birds/listofspecies_corebirds.csv", row.names = FALSE)
```
...
write.csv((bird.list.joined %>% filter(code %in% unique(obs$code))), "C:/Research/CAPLTER/data/core_birds/listofspecies_corebirds_nearbyonly.csv", row.names = FALSE)
```
...
```
One quick correction: the original downstream transect at Ave67 was moved up stream of the original upstream transect between the 2016 Winter and Spring surveys. As a consequence, Ave67_mid_B1 became Ave67_dwn_B1 after 2016, but it is exact same site, so we can rename all the Ave67 observations
...
# before joining with the survey data, the observation data needs to be aggregated
  ...
  obs.date <- obs.surveyID %>% #arrange(survey_id, code) %>% 
    ...
    summarize(bird_count = max(bird_count, na.rm = TRUE)) %>% ungroup()
  ...
  # but those have now been aggregated
```
...
# try joining the observations and survey info
  ...
  surveydays <- surveys %>% group_by(site_code, location_type, survey_date, yday, survey_year, season) %>% 
...
(n.survey <- obs.all %>%
  ...
  filter(code == "ABTO") %>% summarize(n_survey = length(season)) %>% select(-code) %>% arrange(n_survey, survey_year))
...
For occupancy modeling and community compositions analytical approaches, we will likely want to arrange this observation data into as a multi-dimensional array
...
saveRDS(y.count, "./data/bird_observations_countbysurvey.rds")
saveRDS(K, "./data/bird_observations_surveycount.rds")
...
# write.csv(obs.seas, "C:/Research/CAPLTER/data/core_birds/bird_observations_byseasonmax.csv", row.names = FALSE)
write.csv(obs.seas, "./data/bird_observations_countmeanbyseason.csv", row.names = FALSE)
write.csv(obs.all, "./data/bird_observations_cleaned.csv", row.names = FALSE)
