---
title: "CAP Bird Community Dynamics - Visualizing Landscape & Biodiversity Dynamics"
...
ggplot(aes(x = as.factor(survey_year), y = ppt_sum, fill = season)) +
  ...
  geom_boxplot()
```
# DIFFERENCES BETWEEN SEASONS AND AMONG YEARS

### Test for differences in environmental variables over time
```{r differences among seasons}
...
# AMONG-YEAR VARIATION AND TRENDS IN ENVIRONMENTAL DATA

### NDBI trends

```{r among-year variation NDBI}
...
(plot.trend <- env.model %>% filter(!season %in% c("annual")) %>%
  ggplot(aes(x = survey_year, y = NDISI, group = season, color = season)) +
