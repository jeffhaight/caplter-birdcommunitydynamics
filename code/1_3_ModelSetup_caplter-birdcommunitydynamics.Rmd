...
Code for organizing datasets in preparation for statistical modeling
...
### Bird species traits
```{r, echo = TRUE}
birdtraits.all <- read.csv("~/GitHub/caplter-birdcommunitydynamics/data/birdtraits_corebirds2024.csv")
...
obs.all <- read.csv("~/GitHub/caplter-birdcommunitydynamics/data/bird_observations_countmeanbyseason.csv", header = TRUE)
#str(obs.all)

# the abundance data for each bird count (by-survey)
  bird.counts <- readRDS("~/GitHub/caplter-birdcommunitydynamics/data/bird_observations_countbysurvey.rds")
  K <- readRDS("~/GitHub/caplter-birdcommunitydynamics/data/bird_observations_surveycount.rds")
...
Haight, J., De Albuquerque, F. & Frazier, A. Seasonal and annual summary statistics of urbanization, vegetation, land surface temperature, and bioclimatic variables derived from remotely-sensed imagery in areas surrounding long-term bird monitoring locations in the greater Phoenix, Arizona, USA metropolitan area (1997-2023). Environmental Data Initiative https://doi.org/10.6073/PASTA/9D44CD85F881586D6D06E7A7293E833C (2024).

```{r, message = FALSE}
# Import the by-site environmental summary data sourced from the CAP LTER database
data.env <- read.csv("~/GitHub/caplter-birdcommunitydynamics/data/envsummaries_corebirds_combined.csv") %>%
...
# Guild-level Metrics 
  ...
  # Insectivores
    # there are only terrestrial insectivores in this dataset, no aquatic-foraging invertivores
    birdtraits.all %>% filter(common_name %in% c(obs %>% filter(Insectivore == 1) %>% pull(common_name) %>% unique()))

    g.inv <- obs %>% filter(Insectivore == 1) %>%
      summarize(
        n_survey = max(n_survey, na.rm = TRUE), 
        abundance = sum(count_mean), 
        rich = sum(det, na.rm = TRUE), 
        n_guild = length(Insectivore),
      .by = c(site_code, location_type, survey_year, season)
      )
